...
class RFSoC(VisaInstrument):
    ...
    def LUT_and_adress_filling(self, pulses_df, ADC=True, DAC=True):
        ...
        if DAC:
            ...
            DAC_df = pulses_df.loc[pulses_df['module'] == 'DAC']
        ...
        if ADC:
            ...
            ADC_df = pulses_df.loc[pulses_df['module'] == 'ADC']
            LUT_df = ADC_df.loc[ADC_df['rep_tag']==1]
    ...
    def process_sequencing_IQ_table(self, raw_sequence=False):
        ...
        n_clock_cycles_simple = 0
        n_clock_cycles_rep = 0
        pointer_dac = [0,0,0,0,0,0,0,0]
        pointer_adc = [0,0,0,0]
        pulse_after_trig = [False,False,False,False,False,False,False,False]
        prev_mode = [None, None, None, None, None, None, None, None]
        rep_started = False
...
##################################################################################################
        ...
        if max(nb_rep_vec)>1:
            for i in range(8):
                if len(DAC_pulses_pointer[i])>0:
                    nb_pulses_ch[i] = (len(DAC_pulses_pointer[i]) - 1)/2
        ...
        for event_time in event_time_list:
            ...
            for index, row in tmp_df.iterrows():
                ...
                if row['module'] == 'DAC':
                    ...
                    if self.debug_mode:
                        print(not(np.isnan(row['start_pointer'])))
                    ...
                    if not(np.isnan(row['start_pointer'])):
                        ...
                        if not(rep_started):
                            ...
                            while pulses_counter[ch_num - 1]<nb_pulses_ch[ch_num - 1]:
                                pointer_dac[ch_num - 1] +=1
                                pulses_counter[ch_num - 1] +=1 
                    ...
                    if row['mode']=='IQ_mixer':
                        ...
                        if row['mode'] != 'wait':
                            ...
                            for idx, chd in enumerate(ch_demod):
                                ...
                                # add the mixer state and the starting pointer
                                global_sequence = np.append(global_sequence, 4128+(chd - 1)*2)

                                bin_cmd = mux_step[chd - 1]
                                bin_cmd += '1'
                                bin_cmd += '00000000000000'
                                if self.debug_mode:
                                ...
                                if self.debug_mode:
                                    ...
                                    # print(4128+2*idx, int(bin_cmd,2))
            ...
            # Update of the ADC and DAC state
    ...
    def process_sequencing(self, raw_sequence=False):
        ...
        global_sequence = np.array([])
        DAC_pulses_pointer, _ = self.LUT_and_adress_filling(pulses_df, ADC=False)
        ...
        for event_time in event_time_list:
            if event_time>0:
                ...
                #####################################################################################################
                # global_sequence = np.append(global_sequence,int(round((event_time-event_time_prev)*250))-1)

                # if
                wait_time = int(round((event_time-event_time_prev)*250))

                global_sequence = np.append(global_sequence, wait_time)

      #################################################################################################################
                if rep_started:
                    n_clock_cycles_rep += wait_time + 1
                else:
                    n_clock_cycles_simple += wait_time + 1
      ...
      #################################################################################################################
            ...
            for index, row in tmp_df.iterrows():
                ...
                ch_num = int(row['ch_num'])
                if row['mode'] == 'trigger':
                        pulse_after_trig[ch_num - 1]  = False

                if row['module'] == 'DAC':

                    if row['rep_nb']!=1 and not(rep_started):
                        wait_sync = (period_sync-(n_clock_cycles_simple%period_sync)-1)
                        global_sequence = np.append(global_sequence,1)
                        global_sequence = np.append(global_sequence, wait_sync)
                        n_clock_cycles_simple += int(wait_sync + 1)

                        global_sequence = np.append(global_sequence, 257)
                        global_sequence = np.append(global_sequence, row['rep_nb'] - 1)

                        n_clock_cycles_simple +=1
                    ...
                    if self.debug_mode:
                        print(not(np.isnan(row['start_pointer'])), not(pulse_after_trig[ch_num - 1]))


                    if not(np.isnan(row['start_pointer'])) and not(pulse_after_trig[ch_num - 1]):


                        # adding sequencer command to point to address of this pulse
                        if rep_started:
                            n_clock_cycles_rep += 1
                        else:
                            n_clock_cycles_simple += 1
                        ...
                        # TO BE TRANSFORMED FOR RABI LIKE SEQUENCE
                        global_sequence = np.append(global_sequence,DAC_pulses_pointer[ch_num-1][pointer[ch_num - 1]])
                        ...
                        if self.debug_mode:
                            print('adding sequencer command to point to address of this pulse')
                            print(4096+ch_num,DAC_pulses_pointer[ch_num-1][pointer[ch_num - 1]])
                        ...
                        if not(rep_started):
                            pointer[ch_num - 1] +=1
                            if self.debug_mode:
                                print('Pointer shifted to %i'%pointer[ch_num - 1])



                    if not(np.isnan(row['start_pointer'])) and pulse_after_trig[ch_num - 1]:
                        if self.debug_mode:
                            print('Pulse after a trigger detected on channel ' + str(ch_num))
                        pulse_after_trig[ch_num - 1] = not(pulse_after_trig[ch_num - 1])

                    if row['mode'] == 'trigger':
                        pulse_after_trig[ch_num - 1]  = True
                        if self.debug_mode:
                            print('Trigger event detected on channel: %i '%ch_num)

                    if self.debug_mode:
                        print('Pulse after trigger: ' + str(pulse_after_trig))
                ...
                if row['module'] == 'ADC':
                    ...
                    if row['mode'] != 'wait':
                        ...
                        if self.debug_mode:
                            ...
                            print(4096,int(bin_trig_cmd,2))
                            print('Total number of points per cycle: %i' %self.n_points_total)
        ...
        if rep_started:
            wait_term = term_loop
            n_clock_cycles_rep += wait_term + 1
            n_clock_cycles_rep += 1
        else:
            wait_term = int(round((termination_time - last_event)*250))-1
            n_clock_cycles_simple += wait_term + 1
            n_clock_cycles_simple += 1
        ...
        if rep_started:
            wait_sync = int(round(period_sync-(n_clock_cycles_rep%period_sync)-1 - 3)) #(2 clock cycles for jump)
            global_sequence = np.append(global_sequence,1)
            global_sequence = np.append(global_sequence, wait_sync)
            n_clock_cycles_rep += int(wait_sync + 1)
            n_clock_cycles_rep += 2


            # print('REP LOOP waint_sync: %i'%wait_sync)

            global_sequence = np.append(global_sequence, 513)
            global_sequence = np.append(global_sequence, 0)
            n_clock_cycles_rep +=1
         ####################################################################################################
            n_clock_cycles_simple +=3 #???
         ###################################################################################################
        ...
        if self.n_points() > 1:
            ...
            wait_sync = int(round(((period_sync-(n_clock_cycles_rep + n_clock_cycles_simple)%period_sync)-1 - 3))) #(2 clock cycles for jump)
            if self.debug_mode:
                print('WAIT SYNC:' + str(wait_sync))
            ...
            # global_sequence_str = 'SEQ 0,1,9,4106,' + str(acq_mode) + ',258,' + str(int(self.n_points()-1)) + ',' + ','.join((global_sequence.astype(int)).astype(str)) + ',1,' + str(wait_sync) + ',514,0,0,0'
            global_sequence_str = 'SEQ 0,1,9,4106,4128,0,4130,0,4132,0,4134,0,' + str(acq_mode) + ',258,' + str(int(self.n_points()-1)) + ',' + ','.join((global_sequence.astype(int)).astype(str)) + ',1,' + str(wait_sync) + ',514,0,0,0'



        else :
            # global_sequence_str = 'SEQ 0,1,9,4106,' + str(acq_mode) + ',' + ','.join((global_sequence.astype(int)).astype(str)) + ',0,0'
            global_sequence_str = 'SEQ 0,1,9,4106,4128,0,4130,0,4132,0,4134,0,' + str(acq_mode) + ',' + ','.join((global_sequence.astype(int)).astype(str)) + ',0,0'

        if self.debug_mode:
            print('N CLOCK REP: %i'%n_clock_cycles_rep)
            print('N CLOCK SIMPLE: %i'%n_clock_cycles_simple)
    ...
    def time_conversion(self, t):
        ...
        t = np.round(t*nb_clock_per_us)/nb_clock_per_us
        return t 